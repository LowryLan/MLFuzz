...
#include <sys/ioctl.h>
#include <sys/file.h>
/* lowry: add some package from neuzz */
#include <sys/socket.h>
#include <netinet/in.h>


/* lowry: local port to communicate with python module */
# define PORT               12012
...
EXP_ST u8  skip_deterministic,        /* Skip deterministic stages?       */
           ...
           use_splicing = 1,          /* Lowry: recombine input files!    */
...
/* Parameters added by lowry: */
static u32 stop_time    = 1440;	       /* Fuzzing time */
static s32 byte_pos     = 0;           /* Byte position */
// static u64 cycles_done  = 0;           /* fuzz cycles num */

int loc[10000];                        /* weight info array */
...
/* lowry: copy weight_file to read */
...
/* lowry: parse one line of weight_info string into array */
...
static inline u8 has_new_bits(u8* virgin_map) {
  ...
  /* added by lowry: whether stop fuzzing */
...
#define FLIP_BIT(_ar, _b) do { \
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    byte_pos = stage_cur >> 3;    // lowry: 获取当前bit所在字节索引
    /* lowry: 如果权重为负，则跳过，并且令bit位置向后移动7位 */
    if (loc[byte_pos] < 0){
      stage_cur += 7;
      continue;
    }
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:b:t:T:dnCB:S:M:x:QV:k")) > 0)
    ...
    switch (opt) {
      ...
      case 'k': /* added by lowry: fuzzing time */
		    printf("balabala>>>\n");
  ...
  printf("begin to connect py module\n");
  /* lowry: 建立与python的socket通信 */    
  ...
  printf("python module connected !\n");

  char buf[16];

  int boundary;                             // lowry: 读取这么多种子后开启py
  ...
  while (1) {
    ...
    printf("recieve py sign: %s\n", buf);
    /* 将权重文件读取至缓存 */
    char* weight_file = "weight_info_r";
    copy_file("weight_info", weight_file);
    FILE *stream = fopen(weight_file, "r");         // lowry: 将文件读入数据流
    char *line = NULL;                              // lowry: 用于存储文件中每行的信息
    size_t llen = 0;                                
    ssize_t nread;
    /* lowry: 文件读取失败 */
    if (stream == NULL){
    ...
    int line_cnt = 0;                               // lowry: 记录当前读取的行数
    int fflag = 0;
    ...
    while ((nread = getline(&line, &llen, stream)) != -1){
      ...
      /* lowry: 解析文件信息（权重正负|种子文件名） */
      char* weight_forward = strtok(line, "|");        // lowry: 权重正负
      // char* byte_line      = strtok(NULL, "|");     // lowry: 字节长度
      char* fn             = strtok(NULL, "|");        // lowry: 种子文件名
      char* bdr            = strtok(strtok(NULL,"|"),"\n");

      boundary = atoi(bdr);

      if (line_cnt == boundary){
        send(sock, "atten", 5, 0);
      }

      /* lowry: 判断当前所指种子是否为fn，若不是，则指向下一个种子 */
      while (queue_cur && fflag == 0){
        if (strcmp(queue_cur->fname, fn) != 0){
          current_entry++;
          queue_cur = queue_cur->next;
        }else{
          fflag = 1;
      ...
      parse_array(weight_forward, loc);             // lowry: 将weight_forward的string流解析为数组
